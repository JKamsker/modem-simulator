...
| A18 | Replay deterministisch. | Test-sichere Eventlogs fuer Validierung und Playback. | `validate-recompute` prueft Hashes, Initial-State, Sequenzen, Scheduler-Events und bricht bei Divergenz ab; `drive-from-captured-input` und `play-to-dte` scheitern bei Hash-Divergenz ohne Bestaetigung und laufen mit Bestaetigung weiter; `<redacted>`, fehlende Nutzbytes und `replayDivergent=true` scheitern in allen Replay-Modi. | `modemsim replay logs/fixture.jsonl --mode validate-recompute`; `modemsim replay logs/fixture.jsonl --mode drive-from-captured-input --confirm-divergence --case playback-divergence`; `modemsim replay logs/fixture.jsonl --mode play-to-dte --confirm-divergence --case playback-divergence` |
...
| A30 | Source-Size-Gate. | Repo mit kuenstlich uebergrosser Java-Testdatei in isoliertem Fixture. | `mvn verify` bricht auf Linux und Windows/bash-kompatiblen Pfaden, wenn nicht generierter Java-Code `>=300` LOC hat; Ant/Maven ignoriert Exit-Code nicht. | `modemsim test --suite build --case source-size-gate` |
