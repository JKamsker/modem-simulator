...
| Port belegt oder Zugriff verweigert | Startfehler mit klarer Diagnose und Diagnosecode `PORT_BUSY`; kein stiller Retry ohne Konfiguration. |
...
Open-Time-Fehler muessen `not present`, `busy/permission denied` und `unsupported parameters` unterscheiden. `permission denied` wird fuer v1 als `PORT_BUSY` klassifiziert, weil die Recovery- und User-Diagnose identisch ist: Port freigeben oder Rechte korrigieren. Optional aktivierte Sidecar-Ports verwenden dieselben Diagnosecodes und erzeugen `PORT_OPEN_FAILED`-Events mit `source=transport`, `port`, `portRole`, `direction=INTERNAL|NONE` und leerem `rawHex`; bei `strictOptionalPorts=false` bleibt die Hauptsession danach aktiv.
