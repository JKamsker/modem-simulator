...
public final class HeadlessSession implements SessionActor {
    ...
    public synchronized SessionResponse injectDte(RawBytes bytes, String injectionType) {
        int start = eventCount(); long startedNanos = clock.nowNanos(); ModemState before = state; boolean smsBodyEntry = pendingSms != null; RawBytes redactionContext = smsBodyEntry ? bytes : inputState.effectiveCommandLine(bytes);
        events.publishAudit(EventType.INJECTION, Direction.DTE_TO_DCE, injectionType, null, bytes, before, before, smsBodyEntry, redactionContext);
        SessionResponse processed = receive(bytes);
        publishInjectionResult(Direction.DTE_TO_DCE, injectionType, bytes, before, startedNanos, start, smsBodyEntry, redactionContext);
    ...
    public synchronized SessionResponse injectParsedCommand(RawBytes bytes, String injectionType) {
        ...
        events.publishAudit(EventType.INJECTION, Direction.INTERNAL, injectionType, null, bytes, before, before, smsBodyEntry, bytes);
        List<ParsedCommand> commands;
        try {
            commands = parseCommands(bytes);
        } catch (com.jkamsker.modemsim.parser.AtParseException e) { SessionResponse failed = SessionParseFailure.response(profile.errorPolicy(), events, inputState, clock.nowNanos(), state, bytes, RawBytes.empty(), start, smsBodyEntry); publishInjectionResult(Direction.INTERNAL, injectionType, bytes, before, startedNanos, start, smsBodyEntry, bytes); return response(failed.output(), start); }
        ...
        publishInjectionResult(Direction.INTERNAL, injectionType, bytes, before, startedNanos, start, smsBodyEntry, bytes);
    ...
    public synchronized SessionResponse replaceMacroEngine(MacroEngine next, String result) {
        ...
        publishInjectionResult(Direction.INTERNAL, "macro-control", RawBytes.empty(), before, startedNanos, start, false, RawBytes.empty());
        return response(RawBytes.empty(), start);
    }
    ...
    private void publishInjectionResult(Direction direction, String injectionType, RawBytes raw, ModemState before, long startedNanos, int start, boolean smsBodyEntry, RawBytes redactionContext) { events.publishWithLatency(EventType.HANDLER_RESULT, direction, raw, null, before, state, new CommandResult(state, List.of(), null, "Injection:" + injectionType, false), injectionLatency(startedNanos, start), smsBodyEntry, redactionContext); }
